...
def run(s1, s2, k, suff_only, markersize, ax):
    ...
    wp.viz_spy(m, markersize=markersize, title=f"$k$ = {k:,}", verbose=True, ax=ax)
    t2 = time.time()
...
def sim(n, k, markersize=0.5, fig_size_inches=(10, 7)):
    ...
    print_sep()
    axD.set_title(f"common_substrings()\n{tD:,.2f} sec; max mem {memD:,.2f} MiB", fontsize=18)
    axS.set_title(f"suff-only\n{tS:,.2f} sec; max mem {memS:,.2f} MiB", fontsize=18)
